import { useEffect, useState } from 'react';
import axios from 'axios';
import { FaArrowLeft } from 'react-icons/fa';

export function TodosOsEmblemas() {
    const [emblemas, setEmblemas] = useState([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);

    useEffect(() => {
        const fetchEmblemas = async () => {
            try {
                const token = localStorage.getItem('token'); // Obtenha o token do armazenamento local ou do estado global
                const response = await axios.get('http://127.0.0.1:8000/emblemas/lista-todos-emblema/', {
                    headers: {
                        Authorization: `Bearer ${token}`, // Adicione o token no cabeçalho Authorization
                    }
                });
                setEmblemas(response.data);
                setLoading(false);
            } catch (err) {
                setError(err);
                setLoading(false);
            }
        };

        fetchEmblemas();
    }, []);

    if (loading) return <p>Carregando...</p>;
    if (error) return <p>Erro ao carregar emblemas: {error.message}</p>;

    return (
        <div className='mt-[0px] absolute top-[-10px] left-[-550px] pb-20'>
            <h1 className='text-center text-3xl italic font-bold ml-[50px]'>EMBLEMAS</h1>

            <div className='flex pt-20'>
                <a href='/' className='flex flex-col items-center py-1 h-full ml-9'>
                    <FaArrowLeft className='w-10 h-auto' />
                    <p className='w-32 text-sm text-center'>Menu Principal</p>
                </a>

                <div className='flex ml-20'>
                    <a 
                        href='/Emblemas' 
                        className='w-96 h-8 text-black text-xl border-2 border-[#707070] flex justify-center items-center'>
                        Meus Emblemas
                    </a>
                    <a 
                        href='/Todos-os-Emblemas' 
                        className='w-96 h-8 bg-[#0E7886] text-white text-xl border-2 border-[#707070] flex justify-center items-center'>
                        Todos os Emblemas
                    </a>
                </div>
            </div>

            <div className='w-[1120px] h-full mt-8 ml-20 bg-white border-2 border-[#707070] shadow-lg'>
                <div className='h-4 w-full bg-[#0E7886]'></div>

                <div className='flex justify-center items-center my-8'>
                    <div className='grid grid-cols-3 gap-10'>
                        {emblemas.map((item) => (
                            <div 
                                key={item.codigoEmblema} 
                                className={`relative border-2 border-[rgb(112,112,112)] w-[330px] h-full flex flex-col p-4 text-black
                                    ${item.status === 'desbloqueado' ? '' : 'opacity-50 filter grayscale'}`}>
                                <h1 className='absolute -top-2 -right-2 bg-[#0E7886] p-2 text-white'>
                                    {item.codigoEmblema}
                                </h1>
                                <div className='flex'>
                                    <img 
                                        src={item.imagemEmblema} 
                                        alt='icone' 
                                        className='w-20 h-20' />
                                    <div className='p-4'>
                                        <p className='w-40'>{item.tituloEmblema}</p>
                                        <h1 className='text-xl w-48'>{item.subtituloEmblema}</h1>
                                    </div>
                                </div>
                                <p className='pt-4 h-16'>{item.criterio}</p>
                            </div>
                        ))}
                    </div>
                </div>
            </div>
        </div>
    );
}
