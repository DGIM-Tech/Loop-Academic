    ...
    useEffect(() => {
        const fetchEmblemas = async () => {
            try {
                ...
                const meusEmblemas = response.data.filter(item => item.status === 'desbloqueado');
                setEmblemas(meusEmblemas);
                setLoading(false);
            } catch (err) {
                setError(err);
                setLoading(false);
            ...
            <div className='w-[1120px] h-full mt-8 ml-20 bg-white border-2 border-[#707070] shadow-lg'>
                ...
                <div className='flex justify-center items-center my-8'>
                    <div className='grid grid-cols-3 gap-10'>
                        {emblemas.map((item) => (
                            <div key={item.codigoEmblema} className='relative border-2 border-[rgb(112,112,112)] w-[330px] h-full flex flex-col p-4 text-black'>
